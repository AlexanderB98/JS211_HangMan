...
const selectWord = () => {
  ...
  switch (randomNum) {
      case 0:
          ...
          hint = "In mathematics, a statement that the values of two mathematical expressions, where the left hand side and the right hand side are equal"
      case 1:
          word = ["D", "E", "B", "U", "G"]
          hint = "Find and eliminate errors in a computer program"
          break;
      case 2:
          word = ["O", "C", "T", "A", "L"]
      ...
      case 3:
          ...
          hint = "This is generally the detailed organization and implementation of a complex operation"
          break;
      case 4:
          word = ['A', 'N', 'A', 'L', 'O', 'G']
  ...
  for (let i = 0; i < word.length; i++) { wordCorrects[i] = "_" }
}
...
const printBoard = () => {
  removeAllChildNodes();
  document.getElementById("incorrect-count").innerHTML = "Incorrect Guesses: " + incorrectGuess
...
const hangman = (guess) => {
  ...
  window.setTimeout(checkforWin, 1750);
  printBoard()
}
...
const checkforWin = () => {
  ...
  if (incorrectGuess == 0 && counter1 == word.length) {
      confirm("Flawless Victory!! Outstanding!!");//this is similar to alert, but seems to stop code until conformation;
      resetGame();
      console.log("You won without any incorrect guesses!")
  }

  if (incorrectGuess == 10 && counter1 == 0) {
      confirm("You didn't guess a single correct letter? That takes talent...");//this is similar to alert, but seems to stop code until conformation;
      resetGame();
